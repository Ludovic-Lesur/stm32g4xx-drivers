...
#define RTC_REGISTER_MASK_CR    0xE7FFFF7F
#define RTC_REGISTER_MASK_SCR   0x0000003F
...
void __attribute__((optimize("-O0"))) RTC_WKUP_IRQHandler(void) {
    ...
    if (((RTC->SR) & (0b1 << 2)) != 0) {
        ...
        if (((RTC->CR) & (0b1 << 14)) != 0) {
            rtc_ctx.uptime_seconds += STM32G4XX_DRIVERS_RTC_WAKEUP_PERIOD_SECONDS;
...
RTC_status_t RTC_init(RTC_irq_cb_t wakeup_timer_irq_callback, uint8_t nvic_priority) {
    ...
    RCC->BDCR &= ~(0b11 << 8); // Reset bits 8-9.
#if (STM32G4XX_DRIVERS_RCC_LSE_MODE == 0)
    // Use LSI.
    RCC->BDCR |= (0b10 << 8); // RTCSEL='10'.
    ...
    reg_value |= ((STM32G4XX_DRIVERS_RTC_WAKEUP_PERIOD_SECONDS - 1) & RTC_REGISTER_MASK_WUTR);
    RTC->WUTR = reg_value;
    // Register callback.
    ...
    NVIC_set_priority(NVIC_INTERRUPT_RTC_WKUP, nvic_priority);
