...
#define LPUART_REGISTER_MASK_PRESC          0x0000000F

#define LPUART_BRR_VALUE_MIN                0x00300
#define LPUART_BRR_VALUE_MAX                LPUART_REGISTER_MASK_BRR

#if ((STM32G4XX_DRIVERS_RCC_LSE_MODE <= 1) || (defined STM32G4XX_DRIVERS_LPUART_HIGH_BAUD_RATE))
...
static LPUART_status_t _LPUART_set_baud_rate(uint32_t baud_rate) {
...
#elif ((STM32G4XX_DRIVERS_RCC_LSE_MODE == 1) || (defined STM32G4XX_DRIVERS_LPUART_HIGH_BAUD_RATE))
    // Get LSE status.
    ...
    // Set prescaler.
    reg_value = ((LPUART1->PRESC) & (~LPUART_REGISTER_MASK_PRESC));
    reg_value |= (prescaler & LPUART_REGISTER_MASK_PRESC);
    LPUART1->PRESC = reg_value;
    // Set baud rate.
...
#ifdef STM32G4XX_DRIVERS_LPUART_DISABLE_TX_0
